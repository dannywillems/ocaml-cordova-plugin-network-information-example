...
##### PROJECT VARIABLES
PROJECT_NAME				= OCaml-Network-Information
VERSION						= 1.0.0
...
ML_FILES 					=	$(ML_DIRECTORY)/network_information.ml \
								$(ML_DIRECTORY)/dialogs.ml \
								$(ML_DIRECTORY)/test.ml
MLI_FILES 					= 	$(ML_DIRECTORY)/network_information.mli \
								$(ML_DIRECTORY)/dialogs.mli
...
CMI_FILES					=	$(patsubst $(ML_DIRECTORY)/%.mli, $(ML_DIRECTORY)/%.cmi, $(MLI_FILES))
...
$(ML_DIRECTORY)/%.cmi: $(ML_DIRECTORY)/%.mli
	ocamlfind gen_js_api/gen_js_api $<
	ocamlfind ocamlc -package gen_js_api $<
